...
LOAD DATA INFILE 'D:/David/SENA/DiseñoBasesDeDatos/Actividad3ArchivosParaBaseDeDatos/alumnos.txt'
INTO TABLE alumno
...
LOAD DATA INFILE 'D:/David/SENA/DiseñoBasesDeDatos/Actividad3ArchivosParaBaseDeDatos/telefonosContacto.csv'
...
LOAD DATA INFILE 'D:/David/SENA/DiseñoBasesDeDatos/Actividad3ArchivosParaBaseDeDatos/impartir.txt'
INTO TABLE impartir
FIELDS TERMINATED BY '\t'
LINES terminated by '\n'
ignore 1 rows;
...
SELECT
P.idProfesor AS Código,
CONCAT(P.nombre, " ", P.apellido1, " ", P.apellido2) AS NombreCompleto,
P.email AS Correo_Electrónico,
T.telefono,
P.municipio
FROM profesor P
INNER JOIN tlfcontactoprof T ON (P.idProfesor = T.idProfesor)
WHERE P.nombre LIKE "%L%"
AND T.telefono LIKE "L%"*/

SELECT 
P.idProfesor AS Codigo, 
P.nombre AS Nombre,
CONCAT(P.apellido1, " ", P.apellido2) AS Apellido,
P.email AS Correo_Electronico,
T.telefono,
P.municipio
FROM profesor P
INNER JOIN tlfcontactoprof T ON (P.idProfesor = T.idProfesor)
WHERE P.municipio LIKE "%A" 
AND T.telefono LIKE "%6"
-- WHERE P.municipio = "Las Rozas"
-- ORDER BY T.telefono DESC*/

SELECT curso,
nombre,
caracter,
COUNT(nota) AS Alumnos,
ROUND(min(nota), 2) AS 'Nota Minima',
ROUND(max(nota), 2) AS 'Nota Máxima',
ROUND(avg(nota), 2) AS 'Nota Nota Media'
...
/* Asignaturas con una nota media inferior a 5 
orden por curso y posteriormente por nombre
ascendente*/

SELECT matricula.idAsignatura, curso.nombreDescriptivo, asignatura.nombre,  curso.nAsignaturas, AVG(matricula.nota) as promedio FROM matricula
inner join asignatura ON asignatura.idAsignatura = matricula.idAsignatura
inner join curso ON curso.idCurso = asignatura.curso
GROUP BY matricula.idAsignatura
HAVING promedio < 5
